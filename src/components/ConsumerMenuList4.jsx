// src/components/ConsumerMenuList4.jsx
...
const ConsumersMenuList4 = () => {
  ...
  useEffect(() => {
    ...
    const storedCart = localStorage.getItem('cart4'); // Modificar el almacenamiento del carrito
    if (storedCart) {
  ...
  const addToCart = (menu) => {
    ...
    setCart(updatedCart);
    localStorage.setItem('cart4', JSON.stringify(updatedCart));  // Modificar el almacenamiento del carrito
  };
  ...
  const handlePlaceOrder = async () => {
    const orderData = {
      ...
      total: getTotalPrice(),
      source: 'consumers4'  // Añadir el origen del pedido
    };

    try {
      await addOrder(orderData);
      setCart([]);
      localStorage.removeItem('cart4');  // Modificar el almacenamiento del carrito
...
export default ConsumersMenuList4;
