// src/components/ConsumerMenuList7.jsx
...
const ConsumersMenuList7 = () => {
  ...
  useEffect(() => {
    ...
    const storedCart = localStorage.getItem('cart7'); // Modificar el almacenamiento del carrito
    if (storedCart) {
  ...
  const addToCart = (menu) => {
    ...
    setCart(updatedCart);
    localStorage.setItem('cart7', JSON.stringify(updatedCart));  // Modificar el almacenamiento del carrito
  };
  ...
  const handlePlaceOrder = async () => {
    const orderData = {
      ...
      total: getTotalPrice(),
      source: 'consumers7'  // Añadir el origen del pedido
    };

    try {
      await addOrder(orderData);
      setCart([]);
      localStorage.removeItem('cart7');  // Modificar el almacenamiento del carrito
...
export default ConsumersMenuList7;
