// src/components/ConsumerMenuList2.jsx
...
const ConsumersMenuList2 = () => {
  ...
  useEffect(() => {
    ...
    const storedCart = localStorage.getItem('cart2'); // Modificar el almacenamiento del carrito
    if (storedCart) {
  ...
  const addToCart = (menu) => {
    ...
    setCart(updatedCart);
    localStorage.setItem('cart2', JSON.stringify(updatedCart));  // Modificar el almacenamiento del carrito
  };
  ...
  const handlePlaceOrder = async () => {
    const orderData = {
      ...
      total: getTotalPrice(),
      source: 'consumers2'  // Añadir el origen del pedido
    };

    try {
      await addOrder(orderData);
      setCart([]);
      localStorage.removeItem('cart2');  // Modificar el almacenamiento del carrito
...
export default ConsumersMenuList2;
