// src/components/ConsumerMenuList8.jsx
...
const ConsumersMenuList8 = () => {
  ...
  useEffect(() => {
    ...
    const storedCart = localStorage.getItem('cart8'); // Modificar el almacenamiento del carrito
    if (storedCart) {
  ...
  const addToCart = (menu) => {
    ...
    setCart(updatedCart);
    localStorage.setItem('cart8', JSON.stringify(updatedCart));  // Modificar el almacenamiento del carrito
  };
  ...
  const handlePlaceOrder = async () => {
    const orderData = {
      ...
      total: getTotalPrice(),
      source: 'consumers8'  // Añadir el origen del pedido
    };

    try {
      await addOrder(orderData);
      setCart([]);
      localStorage.removeItem('cart8');  // Modificar el almacenamiento del carrito
...
export default ConsumersMenuList8;
