// src/components/ConsumerMenuList3.jsx
...
const ConsumersMenuList3 = () => {
  ...
  useEffect(() => {
    ...
    const storedCart = localStorage.getItem('cart3'); // Modificar el almacenamiento del carrito
    if (storedCart) {
  ...
  const addToCart = (menu) => {
    ...
    setCart(updatedCart);
    localStorage.setItem('cart3', JSON.stringify(updatedCart));  // Modificar el almacenamiento del carrito
  };
  ...
  const handlePlaceOrder = async () => {
    const orderData = {
      ...
      total: getTotalPrice(),
      source: 'consumers3'  // Añadir el origen del pedido
    };

    try {
      await addOrder(orderData);
      setCart([]);
      localStorage.removeItem('cart3');  // Modificar el almacenamiento del carrito
...
export default ConsumersMenuList3;
