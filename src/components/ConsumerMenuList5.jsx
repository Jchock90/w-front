// src/components/ConsumerMenuList5.jsx
...
const ConsumersMenuList5 = () => {
  ...
  useEffect(() => {
    ...
    const storedCart = localStorage.getItem('cart5'); // Modificar el almacenamiento del carrito
    if (storedCart) {
  ...
  const addToCart = (menu) => {
    ...
    setCart(updatedCart);
    localStorage.setItem('cart5', JSON.stringify(updatedCart));  // Modificar el almacenamiento del carrito
  };
  ...
  const handlePlaceOrder = async () => {
    const orderData = {
      ...
      total: getTotalPrice(),
      source: 'consumers5'  // Añadir el origen del pedido
    };

    try {
      await addOrder(orderData);
      setCart([]);
      localStorage.removeItem('cart5');  // Modificar el almacenamiento del carrito
...
export default ConsumersMenuList5;
